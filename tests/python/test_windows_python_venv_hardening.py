...
# install.ps1 and studio/setup.ps1 both open with these three lines, so a real run always emits UTF-8. A script that
# only borrows their functions never executes that preamble, and then the child's stdout is whatever code page the
# host console happens to be on. Pinned here rather than per test: it costs nothing on an ASCII script.
_UTF8_STDOUT = """\
...
def _run_powershell(shell: str, script: str, env: dict[str, str]) -> str:
    ...
    result = run_pwsh(
        [shell, "-NoProfile", "-NonInteractive", "-Command", _UTF8_STDOUT + script],
        check = True,
        capture_output = True,
        text = True,
        # Explicit, because `text = True` alone decodes with the locale encoding, and on Windows that is the ANSI code
        # page (cp1252 on the runners) rather than UTF-8. PowerShell writes the accented path as UTF-8, so the default
        # turned one U+00E4 into U+00C3 U+00A4 and read as install.ps1 corrupting a marker it had restored correctly.
        encoding = "utf-8",
        errors = "strict",
        env = env,
        timeout = 30,
